...
ACTIVE_PATCH="${PATCH_DIR}/0001-gfx1013-compute-queue-fix.patch"
...
# mesa-git and lib32-mesa-git and enable non-interactive user patches.  Fail
# loudly if those upstream defaults change so that the repository can be
# reviewed instead of silently publishing a different package set.
...
# Use CachyOS' native user-patch mechanism.  Only patch 0001 is active;
# mesh/task patches remain disabled release assets.
cp -- "$ACTIVE_PATCH" "$MESA_GIT_BUILD_DIR/mesa-userpatches/0001-gfx1013-compute-queue-fix.mymesapatch"
...
python3 - "$MESA_GIT_BUILD_DIR/PKGBUILD" \
    "$MESA_GIT_PKGREL" "$MESA_GIT_MARCH" "$MESA_GIT_MTUNE" <<'PY'
from pathlib import Path
import re
import sys
...
# so it wins without discarding Arch/CachyOS hardening flags.
pattern = re.compile(
    r'(build \(\) \{\n'
...
MESA_GIT_APPLIED_PATCH=0001-gfx1013-compute-queue-fix.patch
...
printf '    applied patch:            0001-gfx1013-compute-queue-fix.patch\n'
