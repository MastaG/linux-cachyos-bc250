...
# packaging, carrying patches/mesa-testing instead of patches/mesa.
#
# Only vulkan-radeon is emitted, renamed to vulkan-radeon-testing. That is the
...
MESA_TESTING_PKGREL="${MESA_TESTING_PKGREL:-${BC250_PKGREL:-1}}"
...
MESA_TESTING_MARCH="x86-64-v3"
MESA_TESTING_MTUNE="znver2"

[[ "$MESA_TESTING_PKGREL" =~ ^[0-9]+$ ]] || {
    printf 'ERROR: MESA_TESTING_PKGREL must be numeric: %s\n' "$MESA_TESTING_PKGREL" >&2
    exit 1
}
...
mapfile -t MESA_TESTING_PATCHES < <(find "$PATCH_DIR" -maxdepth 1 -type f -name '*.patch' -print | sort)
(( ${#MESA_TESTING_PATCHES[@]} > 0 )) || {
...
rm -rf -- "$MESA_TESTING_BUILD_DIR"
mkdir -p -- "$MESA_TESTING_BUILD_DIR"

printf '==> Downloading CachyOS Mesa PKGBUILD at %s\n' "$CACHYOS_MESA_COMMIT"
curl -fL --retry 5 --retry-all-errors -o "$MESA_TESTING_BUILD_DIR/PKGBUILD" "${UPSTREAM_BASE}/PKGBUILD"
curl -fL --retry 5 --retry-all-errors -o "$MESA_TESTING_BUILD_DIR/gamescope-fps-limiter.patch" \
    "${UPSTREAM_BASE}/gamescope-fps-limiter.patch"

patch_args=()
patch_names=()
for patch in "${MESA_TESTING_PATCHES[@]}"; do
    name="$(basename "$patch")"
    sha256="$(sha256sum "$patch" | awk '{print $1}')"
    b2="$(b2sum "$patch" | awk '{print $1}')"
    cp -- "$patch" "$MESA_TESTING_BUILD_DIR/$name"
    patch_names+=("$name")
    patch_args+=("$name" "$sha256" "$b2")
done

python3 - "$MESA_TESTING_BUILD_DIR/PKGBUILD" "$MESA_TESTING_PKGREL" "$MESA_TESTING_MARCH" "$MESA_TESTING_MTUNE" "${patch_args[@]}" <<'PY'
...
if not re.search(r'^pkgbase=mesa$', text, flags=re.M):
    raise SystemExit('ERROR: expected pkgbase=mesa in Mesa PKGBUILD')
text = re.sub(r'^pkgbase=mesa$', 'pkgbase=mesa-testing', text, count=1, flags=re.M)
...
if text.count('package_vulkan-radeon()') != 1:
    raise SystemExit('ERROR: expected one package_vulkan-radeon() in Mesa PKGBUILD')
text = text.replace('package_vulkan-radeon()', 'package_vulkan-radeon-testing()', 1)

anchor = "  provides=(vulkan-driver)\n  replaces=('amdvlk<=2025.Q2.1-1')\n"
if text.count(anchor) != 1:
    raise SystemExit('ERROR: vulkan-radeon provides/replaces block changed upstream')
text = text.replace(
    anchor,
    "  provides=(vulkan-driver vulkan-radeon=$epoch:$pkgver-$pkgrel)\n"
