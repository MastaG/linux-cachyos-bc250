...
- 4K120 at 4:4:4 over HDMI 2.1, **off by default, opt-in with \`amdgpu.bc250_hdmi21=1\`**: HDMI 2.1 PCON negotiation (\`dcn201-hdmi21-pcon.patch\`) and the two on-die DCN201 DSC engines (\`dcn201-enable-dsc.patch\`) through an active DP 1.4 -> HDMI 2.1 FRL adapter. Verified on a BC-250 into an LG G5 via a UGREEN 8K adapter: FRL PCON negotiated, DSC at 12 bpp, 3840x2160@120 RGB with HDR. **Why it is opt-in:** DSC through an active PCON is not yet reliable across a compositor handover -- switching between a desktop session and gamescope can leave the display dark while the driver reports success at every step, because the adapter's HDMI side dropped and the driver never re-detects the link; only a hotplug recovers it. Without the parameter every touched code path is identical to an unpatched kernel, and a native DisplayPort monitor or a passive adapter is untouched either way. Not upstream -- the work of TeleBooth (https://gist.github.com/TeleBooth/d88ef745895d444a401d0e621de9818e).
- **Display workaround patches withdrawn.** The five \`cs-*\` kernel patches that worked around display faults during HDMI link training are no longer applied; they are kept unapplied in \`patches/<set>/disabled/\`. Those faults only occur with DSC enabled, and DSC is now off by default, so a default install cannot hit them. One conclusion previously published about them was overstated and is corrected in docs/PATCHES.md: the boot black screen was genuinely fixed, but the runtime mode-change case was not -- the release path used \`RequestGfxclk\` (0xE), which has no counterpart and does not actually un-force the clock, so a held override survived the release. A matched \`ForceGfxFreq\`/\`UnForceGfxFreq\` (0x39/0x3A) pair does work, and a separate second failure mode (the PCON dropping its HDMI side on a slow compositor handover, with the driver never re-detecting the link) remains open. Both are why DSC ships opt-in until this is properly fixed.
