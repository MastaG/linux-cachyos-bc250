...
- **Fixed: the display going dark on a mode change** -- most often returning from the desktop to gamescope, sometimes on the first gamescope start after boot. Cause: a GPU governor holding a forced clock/voltage override on the SMU while the display link is brought up. The DP side trains and reports success, but the adapter's HDMI side never comes up, so the screen stays dark until a hotplug. **Not a DSC problem** -- reproduced with \`amdgpu.bc250_hdmi21=0\` on an uncompressed 4-lane HBR2 link with no display patches applied. \`cs-release-gfx-override-at-modeset.patch\` releases the override at the start of any commit that involves a real modeset and restores it afterwards, tunable via \`amdgpu.cs_od_unforce_ms\` (default 3000 ms, 0 disables). Two things were needed and each was established by hardware A/B: the override has to come off (toggling the window 3000 -> 0 -> 3000 in one boot gave pass -> fail -> pass), and the release has to genuinely un-force -- \`RequestGfxclk\` (0xE) has no release counterpart, so the matched \`ForceGfxFreq\`/\`UnForceGfxFreq\` (0x39/0x3A) pair is used instead. This supersedes the five earlier \`cs-*\` workaround patches, which are no longer applied; details and the A/B tables in docs/PATCHES.md.
