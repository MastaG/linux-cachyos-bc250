...
- **Known issue, NOT fixed: the display can go dark on a mode change** -- most often returning from the desktop to gamescope, sometimes on the first gamescope start after boot. Re-seat the cable or trigger a software hotplug (\`echo 0 > /sys/kernel/debug/dri/*/DP-1/trigger_hotplug; sleep 3; echo 1 > ...\`) to recover. **Not a DSC problem** -- reproduced with \`amdgpu.bc250_hdmi21=0\` on an uncompressed 4-lane HBR2 link. Two causes are known: a GPU governor holding a forced clock/voltage override on the SMU while the link is brought up, and the adapter dropping its HDMI side when the stream stays off longer than about a second (the driver then reuses cached state and never re-detects the link, reporting success into a dead link). A kernel fix for the first was shipped in the previous build and has been **withdrawn**: its guard was wrong and it disturbed the SMU at boot even with no governor running, costing the picture at boot. It is unapplied in \`patches/<set>/disabled/\` until the guard is fixed and tested with the governor both enabled and disabled. Running without a GPU governor avoids the first cause entirely. Details and the A/B tables in docs/PATCHES.md.
