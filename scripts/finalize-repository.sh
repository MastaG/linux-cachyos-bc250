...
- 4K120 at 4:4:4 over HDMI 2.1, on by default: HDMI 2.1 PCON negotiation (\`0012\`) and the two on-die DCN201 DSC engines (\`0013\`) through an active DP 1.4 -> HDMI 2.1 FRL adapter. Verified on a BC-250 into an LG G5 via a UGREEN 8K adapter: FRL PCON negotiated, DSC at 12 bpp, 3840x2160@120 RGB with HDR. A native DisplayPort monitor or a passive adapter is untouched either way. \`amdgpu.bc250_hdmi21=0\` switches it off and gives an unpatched kernel back. Not upstream -- the work of TeleBooth (https://gist.github.com/TeleBooth/d88ef745895d444a401d0e621de9818e). Known and unrelated: some DP->HDMI 2.1 adapters show black from kernel takeover until a hotplug, with the feature off as well; under investigation, workaround in docs/PATCHES.md.
