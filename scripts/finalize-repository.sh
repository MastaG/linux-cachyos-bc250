...
- **Solved:** the black-screen-at-boot issue some DP->HDMI 2.1 adapters (and native HDMI 2.1 FRL displays) hit, where the screen stayed black from kernel takeover until a hotplug, or synced late enough to miss the boot splash. Root cause: \`cyan-skillfish-governor\` (or any tool writing to \`pp_od_clk_voltage\`) sends a forced GPU clock/voltage override to the SMU on its first cycle, and if that lands while the HDMI FRL link is still training, it desyncs the link -- confirmed on real hardware by reproducing the symptom with an isolated manual write, no governor involved. Fixed by four kernel patches. Three \`cs-defer-*\` ones hold GPU clock/voltage commits off while a link is being brought up, which fixes the black screen at boot. A **runtime** mode change needed more: isolated on hardware (governor stopped in both cases, no commit anywhere near the modeset), the trigger is a \`ForceGfxVid\` override being *held*, not the commit -- so \`cs-release-gfx-override-during-link-bringup.patch\` releases it for the bring-up and restores it afterwards. \`amdgpu.cs_od_unforce_ms\` tunes that window (default 3000 ms, 0 disables). Details and the A/B tables in docs/PATCHES.md. The PCON half originally waited on the converter's own readiness bits for up to 10 s; that polling turned out to be breaking the very training it waited for -- it added ~20 s to boot and corrupted 4K120 12-bit HDR on some adapters -- and was replaced by a non-blocking deadline (\`amdgpu.cs_pcon_frl_defer_ms\`, default 2000 ms). Verified on hardware across three kernels: details and the A/B table in docs/PATCHES.md.
