...
The upstream `bc250-fsr4-v3.patch` is trimmed while rebasing onto this tree, and the patch header records each change. It no longer touches ACO at all: the GFX1013 compute-queue and `RADV_GFX103` hunks duplicated `0001` and `0004`, the MAD forms are emitted as plain `iadd(imul24_relaxed(..), ..)` because ACO already fuses that pair into `v_mad_i32_i24` itself, the explicit wrapper rules duplicated folds Mesa performs anyway, and an LDS spill-slot override was dropped because on GFX1013 it reserved 25.5 KiB of LDS per workgroup and admitted only 1 wave/SIMD, against the 6 waves/SIMD it was meant to protect. The deferred round is also gated on the shader actually containing a packed signed dot product, so ordinary GFX1013 compute shaders no longer pay for a second optimization round.
